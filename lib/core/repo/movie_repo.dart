...
abstract class MovieRepo {
  ...
  Future<PopularMovieModel> getHighestRatedMovie({required int movieId});
}
...
class MovieRepoImpl extends MovieRepo {
  ...
  Future<PopularMovieModel> getPopularMovie() async {
    final response =
        await _networkService.getData(url: ApiRoutes.getPopularMovie(id: "!"));
  ...
  @override
  Future<PopularMovieModel> getHighestRatedMovie({required int movieId}) async {
    final response = await _networkService.getData(
        url: ApiRoutes.getHighestRatedMovie(page: 1));
    return PopularMovieModel.fromJson(response);
  }
}
